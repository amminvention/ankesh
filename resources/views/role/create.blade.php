            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form class="cmxform" method="POST" action="{{ route('role-permission.store') }}">
                            @csrf
                            <fieldset>
                                    ...
                                    @foreach($permissions as $id => $name)
                                        <div class="form-check">
                                            <label class="form-check-label">
                                                <input type="checkbox" class="form-check-input" name="permission[]" value="{{ $id }}" {{ old('permission') == $id ? 'checked' : '' }}>
                                                {{ ucwords($name) }}
                                            </label>
                                        </div>
                                    @endforeach
